function res = MCMC_Gibbs_Sampler_Beads(setup)
...
cor_mean_prior = setup.cor_mean_prior;
cor_sigma_prior = setup.cor_sigma_prior;
...
cor_sigma_proposal = setup.cor_sigma_proposal;
...
if saveRecons==1
    ...
    %Only do a running sum of the samples
    x_sum =         zeros(N^2,1);
end
...
if COR == 1
    cor_samps = zeros(1,N_samples);      %Array for dx samples
    cor = cor0;                           %Initial dx estimate
    n_accept_cor = zeros(1,N_samples);                    %Metropolis acceptance rate tracker
else
    cor = cor_true;                       %If we do not estimate the parameter, we set it to the true value
end
...
for k=1:N_samples
    ...
    if SOURCE_X == 1
        ...
        for j = 1:N_metro
            ...
            proj_geom = astra_create_proj_geom('fanflat_vec',p,vectors);
            A_prop = opTomo('cuda',proj_geom,vol_geom);
    ...
    if COR == 1
        ...
        for j = 1:N_metro
            %Get candidate sample from proposal
            cor_prop = normrnd(cor,cor_sigma_proposal);
        
            vectors = zeros(length(angles),6);
        
            %Specify vectorized geometry
            vectors(:,1) = cos(angles)*cor_prop-sin(angles)*sy;
            vectors(:,2) = sin(angles)*cor_prop+cos(angles)*sy;
            vectors(:,3) = cos(angles)*cor_prop+sin(angles)*dy;
            vectors(:,4) = sin(angles)*cor_prop-cos(angles)*dy;
            vectors(:,5) = cos(angles + tilt*pi/180)*detector_width/p;
            vectors(:,6) = sin(angles + tilt*pi/180)*detector_width/p;
            ...
            %Ensure more numerical stability by doing logpdf
            lp_old = -lambda/2*norm(Ax-b,2)^2-1/2*(cor-cor_mean_prior)^2/cor_sigma_prior^2;
            Apropx = A_prop*x;
            lp_prop = -lambda/2*norm(Apropx-b,2)^2-1/2*(cor_prop-cor_mean_prior)^2/cor_sigma_prior^2;
        ...
        if mod(k,100)==0
            disp(['New cor sample: ' num2str(cor)])
            disp(['Number of accepted Metropolis for dx sampling: ' num2str(count)])
        end
        cor_samps(k) = cor;
        n_accept_cor(k) = count;
    end
    if TILT == 1
        count = 0;
        %Sample detector y parameter using Metropolis-Hastings 
    ...
    options.x0 = x;
    x = lsqr_gentikh_fixedNorm(A,D,b_tilde,u,lambda,delta,normA,options);
    Ax = A*x;
...
if COR == 1
    res.cor_samps = cor_samps;
    res.n_accept_cor = n_accept_cor;
end
